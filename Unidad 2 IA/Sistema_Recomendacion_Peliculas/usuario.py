...
class usuario:
    ...
    def ver_pelicula(self,pelicula,lista_peliculas):
        if pelicula not in self.pelis_vistas:
            ...
            ##Aqui fue donde implemente la persistencia basicamente cuando el
            ## usuario ve una pelicula se sobreescribe el archivo aniadiendo la id
            ##de la pelicula a su lista de vistas en u.user
            lista_filas=[]
            with open("data/u.user","r",encoding="utf-8") as datos:
                for filas in datos:
                    partes=filas.strip().split("|")
                    if partes[0]==self.id:
                        lista_filas.append(f"{partes[0]}|{partes[1]}|{",".join(str(pelis.id) for pelis in self.pelis_vistas)}\n")
                    else:
                        lista_filas.append(filas)
            with open("data/u.user", "w", encoding="utf-8") as datos:
                datos.writelines(lista_filas)        
